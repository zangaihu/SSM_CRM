										...
										<tr class="message-unread">
											...
											<td class="hidden-xs">
												<c:if test="${cv.status=='已打款'}">
													<span class="badge badge-success mr10 fs11">${cv.status}</span>
												</c:if>
												<c:if test="${cv.status=='新创建'}">
													<span class="badge badge-warning mr10 fs11">${cv.status}</span>
												</c:if>
												<c:if test="${cv.status=='已提交'}">
													<span class="badge badge-hero mr10 fs11">${cv.status}</span>
												</c:if>
												<c:if test="${cv.status=='已审核'}">
													<span class="badge badge-info mr10 fs11">${cv.status}</span>
												</c:if>
												<c:if test="${cv.status=='已打回'}">
													<span class="badge badge-alert mr10 fs11">${cv.status}</span>
												</c:if>
												<c:if test="${cv.status=='已终止'}">
													<span class="badge badge-danger mr10 fs11">${cv.status}</span>
												</c:if>
												<c:if test="${cv.status=='待复审'}">
													<span class="badge badge-primary mr10 fs11">${cv.status}</span>
												</c:if>

						...
						<div class="panel heading-border">
							<div class="panel-body bg-gray-light">
								...
								<div class="section row">
									...
									<div class="col-md-4"><fmt:formatDate  pattern="yyyy-MM-dd" value="${claimVoucher.createTime}"></fmt:formatDate></div>
								</div>
								<div class="section row">
									...
									<div class="col-md-4">
										<c:if test="${claimVoucher.status=='已打款'}">
											<span class="label label-success">${claimVoucher.status}</span>
										</c:if>
										<c:if test="${claimVoucher.status=='新创建'}">
											<span class="label label-default">${claimVoucher.status}</span>
										</c:if>
										<c:if test="${claimVoucher.status=='已提交'}">
											<span class="label label-primary">${claimVoucher.status}</span>
										</c:if>
										<c:if test="${claimVoucher.status=='已审核'}">
											<span class="label label-info">${claimVoucher.status}</span>
										</c:if>
										<c:if test="${claimVoucher.status=='已打回'}">
											<span class="label label-warning">${claimVoucher.status}</span>
										</c:if>
										<c:if test="${claimVoucher.status=='已终止'}">
											<span class="label label-danger">${claimVoucher.status}</span>
										</c:if>
										<c:if test="${claimVoucher.status=='待复审'}">
											<span class="label label-danger">${claimVoucher.status}</span>
										</c:if>
									</div>
								</div>
								...
								<div class="section row">
									<c:forEach items="${records}" var="record">
										...
										<div class="col-md-3"><fmt:formatDate  pattern="yyyy-MM-dd" value="${record.dealTime}"></fmt:formatDate></div>
